...
const CHAT_HIDDEN_PREF_KEY='courtrush_hidden_chat_messages_v1';
...
function friendChatReadKey(playerId){ return `${state.currentUser?state.currentUser.uid:'guest'}:friend:${friendChatRoomId(playerId)}`; }
function hiddenChatState(){
  try{ return JSON.parse(localStorage.getItem(CHAT_HIDDEN_PREF_KEY)||'{}')||{}; }
  catch(_){ return {}; }
}
function hiddenChatKey(clubId){ return `${state.currentUser?state.currentUser.uid:'guest'}:${clubId}`; }
function hiddenChatMessageIds(clubId){
  const ids=hiddenChatState()[hiddenChatKey(clubId)];
  return new Set(Array.isArray(ids)?ids:[]);
}
function rememberHiddenChatMessageIds(clubId,ids){
  const stateMap=hiddenChatState();
  stateMap[hiddenChatKey(clubId)]=[...new Set(ids)].slice(-1000);
  try{ localStorage.setItem(CHAT_HIDDEN_PREF_KEY,JSON.stringify(stateMap)); }catch(_){}
}
...
function visibleMessagesForClub(clubId){
  const hidden=hiddenChatMessageIds(clubId);
  return allChatMessages().filter(message=>message.clubId===clubId&&canViewChatMessage(message)&&!hidden.has(message.id));
}
...
function friendRequestPairId(a,b){ return `friend_${[String(a||''),String(b||'')].sort().join('_')}`; }
function friendRequestsForMe(){
  if(!state.myPlayerId) return [];
  return state.friendRequests.filter(request=>request.fromPlayerId===state.myPlayerId||request.toPlayerId===state.myPlayerId);
}
function friendRequestWith(playerId){
  const id=friendRequestPairId(state.myPlayerId,playerId);
  return friendRequestsForMe().find(request=>request.id===id)||null;
}
function acceptedFriendIds(){
  if(!state.myPlayerId) return [];
  return friendRequestsForMe()
...
function unreadFriendMessageCount(playerId){
  const lastRead=Number(chatReadState()[friendChatReadKey(playerId)]||0);
  return friendChatMessagesFor(playerId).filter(message=>message.senderPlayerId!==state.myPlayerId&&chatMessageTime(message)>lastRead).length;
}
function totalUnreadSocialCount(){
  return totalUnreadMentions()+acceptedFriendIds().reduce((total,playerId)=>total+unreadFriendMessageCount(playerId),0);
}
...
function markFriendChatRead(playerId){
  if(!playerId||!state.currentUser) return;
  const reads=chatReadState();
  reads[friendChatReadKey(playerId)]=Date.now();
  try{ localStorage.setItem(CHAT_READ_PREF_KEY,JSON.stringify(reads)); }catch(_){}
}
...
function stopFriendRequestSync(){
  friendRequestUnsubs.forEach(unsub=>unsub());
  friendRequestUnsubs=[];
...
function filterVisibleRosterTiles(){
  const query=normalizePlayerSearch(state.rosterSearchQuery||'');
  document.querySelectorAll('.roster-member-tile[data-roster-search]').forEach(tile=>{
    const haystack=tile.dataset.rosterSearch||'';
    tile.hidden=!!query&&!query.split(/\s+/).every(term=>haystack.includes(term));
  });
}
...
function notificationEmailTemplate({type,title,body,url,recipientName}){
  const actionUrl=url||'./index.html';
  const greeting=recipientName?`Hi ${recipientName},`:'Hi CourtRush player,';
  const templates={
    friend_request:{
      subject:title||'New CourtRush friend request',
      text:`${greeting}\n\n${body}\n\nOpen CourtRush Social to accept or decline the request:\n${actionUrl}`
    },
    friend_accepted:{
      subject:title||'Friend request accepted on CourtRush',
      text:`${greeting}\n\n${body}\n\nOpen CourtRush Social to continue:\n${actionUrl}`
    },
    friend_message:{
      subject:title||'New CourtRush friend message',
      text:`${greeting}\n\n${body}\n\nOpen CourtRush Social to reply:\n${actionUrl}`
    },
    schedule_created:{
      subject:title||'You were added to a CourtRush Game Plan',
      text:`${greeting}\n\n${body}\n\nOpen CourtRush Game Plan for court assignments and updates:\n${actionUrl}`
    },
    schedule_updated:{
      subject:title||'CourtRush Game Plan updated',
      text:`${greeting}\n\n${body}\n\nOpen CourtRush Game Plan to review the update:\n${actionUrl}`
    },
    club_invite:{
      subject:title||'CourtRush club invitation',
      text:`${greeting}\n\n${body}\n\nOpen CourtRush to respond:\n${actionUrl}`
    }
  };
  return templates[type]||{subject:title||'CourtRush notification',text:`${greeting}\n\n${body}\n\n${actionUrl}`};
}
async function createPlayerNotifications({type,sourceId,clubId,playerIds,title,body,url,extra={}}){
  ...
  const writeNotification=playerId=>{
    ...
    const email=playerEmail(recipient);
    const emailTemplate=notificationEmailTemplate({type,title,body,url,recipientName:playerDisplayName(recipient)});
    ...
    const record={
      ...
      delivery:{
        ...
        email:{status:email?'queued':'unavailable',to:email||null,subject:emailTemplate.subject,body:emailTemplate.text}
      },
      ...extra
    };
    return {id,record};
...
function deleteChatMessageForMe(clubId,messageId){
  if(!clubId||!messageId) return;
  const ids=hiddenChatMessageIds(clubId);
  ids.add(messageId);
  rememberHiddenChatMessageIds(clubId,ids);
  toast('Message deleted on your end');
  render();
}
function clearClubChatForMe(clubId){
  const club=clubById(clubId);
  if(!club) return;
  const messages=allChatMessages().filter(message=>message.clubId===clubId&&canViewChatMessage(message));
  if(!messages.length){ toast(`${club.name} Social is already empty on your end`); return; }
  if(!confirm(`Delete ${club.name} Social on your end only? Other members will still keep the chat.`)) return;
  rememberHiddenChatMessageIds(clubId,messages.map(message=>message.id));
  toast(`${club.name} Social deleted on your end`);
  render();
}
async function sendFriendRequest(playerId){
  if(!state.currentUser||!state.myPlayerId){ toast('Sign in and link your player profile before adding friends'); return; }
  ...
  try{
    if(existing&&existing.status==='declined'){
      await FRIEND_REQUESTS_COL.doc(id).set({status:'pending',updatedAt:now},{merge:true});
    }else{
    ...
    await createPlayerNotifications({
      ...
      body:`${playerDisplayName(state.players.find(player=>player.id===state.myPlayerId))} sent you a friend request on CourtRush.`,
      url:'./index.html#social',
      extra:{fromPlayerId:state.myPlayerId,toPlayerId:playerId}
    });
    ensureNotificationPermission();
    toast(`Friend request sent to ${playerName(playerId)}`);
  }catch(e){ console.error(e); toast(`Could not send the friend request${e&&e.code?` (${e.code})`:''}. Publish the updated Firestore rules and try again.`); }
...
async function respondFriendRequest(requestId,accept){
  ...
  const now=new Date().toISOString();
  const update=accept?{status:'accepted',acceptedAt:now,updatedAt:now}:{status:'declined',declinedAt:now,updatedAt:now};
  state.friendRequestBusyId=request.fromPlayerId;
  render();
  try{
    await FRIEND_REQUESTS_COL.doc(request.id).set(update,{merge:true});
    state.friendRequests=state.friendRequests.map(item=>item.id===request.id?{...item,...update}:item);
    if(accept){
      await createPlayerNotifications({
        type:'friend_accepted',
        sourceId:request.id,
        playerIds:[request.fromPlayerId],
        title:'Friend request accepted',
        body:`${playerDisplayName(state.players.find(player=>player.id===state.myPlayerId))} accepted your friend request. They now appear in Social.`,
        url:'./index.html#social',
        extra:{fromPlayerId:request.fromPlayerId,toPlayerId:request.toPlayerId}
      });
    }
...
function renderTopbar(){
  ...
  const unreadChatMentions=totalUnreadSocialCount();
...
function setSocialSearchQuery(value){
  state.socialSearchQuery=String(value||'').trimStart().slice(0,80);
  filterVisibleSocialConversations();
}
function applySocialSearchQuery(value){
  state.socialSearchQuery=String(value||'').trimStart().slice(0,80);
  render();
}
function filterVisibleSocialConversations(){
  const query=String(state.socialSearchQuery||'').trim().toLowerCase();
  let shown=0,total=0;
  document.querySelectorAll('.chat-club-button[data-social-search]').forEach(button=>{
    total+=1;
    const match=!query||(button.dataset.socialSearch||'').includes(query);
    button.hidden=!match;
    if(match) shown+=1;
  });
  const count=document.querySelector('.social-search-count');
  if(count) count.textContent=String(shown);
}
function filterVisibleClubCards(){
  const query=String(state.clubSearchQuery||'').trim().toLowerCase();
  let shown=0,total=0;
  document.querySelectorAll('.club-card[data-club-search]').forEach(card=>{
    total+=1;
    const match=!query||(card.dataset.clubSearch||'').includes(query);
    card.hidden=!match;
    if(match) shown+=1;
  });
  const count=document.querySelector('.club-search-count');
  if(count) count.textContent=`${shown}/${total} club${total===1?'':'s'} shown`;
}
  ...
  <section class="club-search-panel" aria-label="Search clubs">
    <div>
      ...
      <label class="club-search">
        ...
        <input type="search" value="${esc(state.clubSearchQuery||'')}" placeholder="Search club name, origin, or members" aria-label="Search clubs" oninput="setClubSearchQuery(this.value)" onkeydown="if(event.key==='Enter')applyClubSearchQuery(this.value)" onblur="applyClubSearchQuery(this.value)" />
      </label>
    </div>
    <span class="small muted club-search-count">${clubs.length}/${allClubs.length} club${allClubs.length===1?'':'s'} shown</span>
  </section>
...
function renderClubChat(){
  ...
  if(activeFriend) markFriendChatRead(activeFriend.id);
...
function renderRosterMemberTile(row){
  ...
  const searchText=normalizePlayerSearch([displayName,realName,nickname,email,p.playerId,activePlayerClubIds(p).map(clubName).join(' ')].filter(Boolean).join(' '));
  const friendAction=renderFriendAction(p);
  return `<article class="roster-member-tile" data-roster-search="${esc(searchText)}" onclick="openPlayerProfile(${jsArg(p.id)})">
    ...
    <div class="section-title roster-heading">
      <div class="roster-heading-main">
        ...
        <label class="roster-search">
          ...
          <input type="search" value="${esc(state.rosterSearchQuery||'')}" placeholder="Search members, nicknames, email, or clubs" aria-label="Search members" oninput="setRosterSearchQuery(this.value)" onkeydown="if(event.key==='Enter')applyRosterSearchQuery(this.value)" onblur="applyRosterSearchQuery(this.value)" />
        </label>
      </div>
          ...
          return `<div class="court-card">
            ...
            <div class="court-body">
              ...
              ${recordedMatch ? (state.editingResultId===recordedMatch.id ? `
                ...
                <div class="score-inputs">
                  <span class="score-result-text">${recordedMatch.score1} <span class="score-vs">-</span> ${recordedMatch.score2}</span>
